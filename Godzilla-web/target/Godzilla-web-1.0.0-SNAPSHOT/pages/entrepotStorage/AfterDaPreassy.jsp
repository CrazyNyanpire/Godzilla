...
Array.prototype.debtWafercheck=Array.prototype.debtWafercheck ||function(id,value)
{
	for(var b in this){
		if(id==this[b]['lotId'])
			if(value==this[b]['waferId'])
				{
					return false;
				}
		}
	         ...
	         return grid.grid({
	                ...
	                columns: [
		                      ...
		                      { title: 'Strip #', name: 'stripQtyIn', width: 60},
   	                          ...
   	                          { title: 'Equipment', name: 'equipment', width: 115},
		                      ...
		                      { title: 'Lot Type', name: 'lotType', width: 80},
   	                          ...
   	                          { title: 'Pack S.', name: 'packSize', width: 90},
                       ...
                       json = json.data;
                        ...
                        .done(function(msg) {
                        	...
                        	for(var i=0;i<msg.length;i++)
                        		{
                        			dialog.find("#God_Track_In_append").append("<tr><td><input type='radio' name='equipmentId' value='"+msg[i]['id']+"'></td><td style='text-align:left;'>"+msg[i]['equipment']+"</td><td style='text-align:left;'>"+msg[i]['discription']+"</td><td style='text-align:left;'>" + msg[i]['stationName'] +"</td><td style='text-align:left;'>"+msg[i]['status']+"</td><td style='text-align:left;'>"+msg[i]['runlots']+"</td></tr>");
                        		}
	    ...
	    Track_Out: function(id, grid){
	        ...
	        $.get('<%=path%>/AfterDaWaferTrackOut.jsp').done(function(html){
	               ...
	               dialog.find('.God_debit_waferAdd').bind("click",function(){
	            	   		...
	            	   		if(!waferDebit.debtWafercheck(funddatalistoption('God_debit_lotNumberList','God_debit_lotNO').attr('id'),funddatalistoption('God_debit_waferIdList','God_debit_waferId').attr('id')))
	            	   			{return false;}
	            	   		...
	            	   		$.get( '${pageContext.request.contextPath}/WaferProcess/get/' + funddatalistoption('God_debit_lotNumberList','God_debit_lotNO').attr('id') + '.koala').done(function(json){
								...
								k++;
								$('.God_debit_lotNO').val("");
		            	   		$('.God_debit_waferId').val("");
		            	   		$("input[name^='wafer']").unbind();
		            	   		$("input[name^='wafer']").bind("blur",function(){
		            	   			debugger;
	               ...
	               $.get( '${pageContext.request.contextPath}/ManufactureProcess/get/' + id + '.koala').done(function(json){
	                ...
	                dialog.find('#God_next_split1').on('click',{grid: grid}, function(e){
	                    if(!Validator.Validate(dialog.find('form')[0],3))return;
	                    debugger;
	                    if($("input[name^='wafer']").length>0)
	                    	{
	                    		for(var i=1;i<=$("input[name^='wafer']").length;i++)
	                    			{
	                    				if(parseInt($("input[name^='wafer']").eq(i).val())>parseInt($("input[name^='wafer']").eq(i).parent().prev().text()))
	                    					{
	                    						return;
	                    					}
	                    			}
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    if(!Validator.Validate(dialog.find('form')[0],3)||flag)return;
