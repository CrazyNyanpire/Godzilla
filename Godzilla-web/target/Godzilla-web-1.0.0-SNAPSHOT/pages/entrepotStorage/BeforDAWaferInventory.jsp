	...
	PageLoader = {
	   //
	    ...
	    Start: function(id, grid){
	        ...
	        var waferIdLength=0;
	        var waferinfo=[];
	        var dialog = $('<div class="modal fade"><div style="width: 100%;" class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">Start</h4></div><div class="modal-body"><p>One fine body&hellip;</p></div><div class="modal-footer"><button type="button" class="btn btn-default" style="display:none" id="God_prev_split">上一步</button><button type="button" class="btn btn-default" data-dismiss="modal" id="God_cancel_split">取消</button><button type="button" class="btn btn-success" id="God_next_split">下一步</button><button type="button" class="btn btn-success" id="save" style="display:none">提交</button></div></div></div></div>');
	        $.get('<%=path%>/WaferStart.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	               self.initPage(dialog.find('form'));
	               if(AlphaId=='203'){
	            	   dialog.find('#startFlowID').parent().parent().remove();
	            	   dialog.find(".God_waferStartShow").remove();
	            	   dialog.find(".God_waferStartHide input").attr('dataType','Require');
	            	   dialog.find(".God_waferStartHide textarea").attr('dataType','Require');
	            	   dialog.find(".God_waferStartHide").css("display","initial");
	            	   dialog.find("#God_cancel_split").css("display","initial");
	            	   dialog.find("#save").css("display","initial");
	            	   dialog.find("#God_next_split").css("display","none");
	            	   dialog.find("#God_prev_split").css("display","none");
	               }
	               $.get( '${pageContext.request.contextPath}/WaferProcess/get/' + id + '.koala').done(function(json){
                       json = json.data;
                        var elm;
	    			...
	    			$.get('/WaferInfo/getWaferInfo/'+id+'.koala').done(function(json){
	    				...
	    				for(var i=0;i<json.length;i++)
	    					{
	    						dialog.find('.God_waferId').append('<tr><td style="vertical-align: middle;">Wafer'+k+':</td><td><input name="Wafer'+k+'" data_value="'+json[i]['id']+'" readonly style="display:inline; width:94%;" class="form-control" Placeholder="Input WaferId..." type="text"/></td><td><input name="WaferQty'+k+'" readonly style="display:inline; width:94%;" class="form-control" Placeholder="Input WaferQty..." type="text" /></td></tr>');
	    						k++;
	    					}
	    	            var elm;
	    	            k=1;
	    	            for(var i=0;i<json.length;i++){
	    	                elm = dialog.find("input[name='Wafer"+k+"']");
	    	                elm.val(json[i]['waferId']);
	    	                elm = dialog.find("input[name='WaferQty"+k+"']");
	    	                elm.val(json[i]['dieQty']);
	    	                k++;
	    	            }
		    			var array=dialog.find(".God_waferId input");
		    			for(var a in array){
		    				if(array.eq(a).val().split("Dummy")[1]!=undefined)//修改waferId关键词过滤
	    					{
		    					debugger;
	    						array.eq(a).removeAttr('readonly');
	    						dialog.find("input[name='WaferQty"+array.eq(a).attr('name').replace(/[^0-9]/ig, '')+"']").removeAttr('readonly');
	    					}
		    			}
	    			});
	                dialog.find('#God_next_split').on('click',{grid: grid}, function(e){
	                    if(!Validator.Validate(dialog.find('form')[0],3))return;
   	                	$(".God_waferStartHide").css("display","initial");
   	                	$(".God_waferStartShow").css("display","none");
   	                    ...
   	                    $("#God_prev_split").css("display","initial");
   	                 	$(".God_waferStartHide input").attr('dataType','Require');
   	                 	$(".God_waferStartHide textarea").attr('dataType','Require');
   	                 	debugger;
   	                 	for(var j=1;j<=waferIdLength;j++)
 							{
								var waferinfojson={};
								waferinfojson['waferId']=dialog.find(".God_waferId input[name=Wafer"+j+"]").val();
								waferinfojson['waferQty']=dialog.find(".God_waferId input[name=WaferQty"+j+"]").val();
								waferinfojson['id']=dialog.find(".God_waferId input[name=Wafer"+j+"]").attr("data_value");
								waferinfo.push(waferinfojson);
 							}
	                    });
	                dialog.find('#God_prev_split').on('click',{grid: grid}, function(e){
	                    $(".God_waferStartShow").css("display","initial");
	                    $(".God_waferStartHide").css("display","none");
	                    $("#God_cancel_split").css("display","initial");
	                    $("#save").css("display","none");
	                    $("#God_next_split").css("display","initial");
	                    $("#God_prev_split").css("display","none");
	                    $(".God_waferStartHide input").attr('dataType','');
	                    $(".God_waferStartHide textarea").attr('dataType','');
	                });
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    if(!Validator.Validate(dialog.find('form')[0],3))return;
	                    var rs=requestParamToJson(dialog.find('form').serializeArray());
