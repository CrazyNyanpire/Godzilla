	...
	*/
	...
	PageLoader = {
	   //
	    ...
	    initGridPanelone: function(){
	         ...
	         return grid.grid({
	                ...
	                columns: [
		                      ...
		                      { title: 'Step', name: 'rejectStation', width: width},
	    ...
	    Start: function(id, grid){
	        var self = this;
	        var dialog = $('<div class="modal fade"><div style="width: 32%;" class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">Track Out</h4></div><div class="modal-body"><p>One fine body&hellip;</p></div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">取消</button><button type="button" class="btn btn-success" id="save">提交</button></div></div></div></div>');
	               ...
	               $.get( '${pageContext.request.contextPath}/ManufactureProcess/get/' + id + '.koala').done(function(json){
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    ...
	                    rs['id']=id;
	  	          		var restring=JSON.stringify(rs);
	  	          		debugger;
	                    $.post('${pageContext.request.contextPath}/ManufactureProcess/Process/semiStart.koala', "data="+restring).done(function(result){
	                        if(result.result == 'success'){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
	        ...
	        $.get('<%=path%>/AfterDaWaferSplitLot.jsp').done(function(html){
	               ...
	               $.get( '${pageContext.request.contextPath}/ManufactureProcess/get/' + id + '.koala').done(function(json){
	                ...
	                dialog.find('#God_next_split').on('click',{grid: grid}, function(e){
	                    ...
	                    var inobj={};
	                    inobj['id']=id;
	                    inobj['waferQty']=parseInt(rs['waferQty']);
	                    inobj['dieQty']=parseInt(rs['dieQty']);
	                    inobj['comments']=rs['comments'];
	                    inobj['locationIds']=rs['locationIds'];
	                    rs['data']=inobj;
	                    delete rs['comments'];
	    ...
	    Get_Traveler: function(id, grid){
	        ...
	        $.get('<%=path%>/AfterDaWaferGetTraveler.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	               self.initPage(dialog.find('form'));
	               $.get( '${pageContext.request.contextPath}/WaferProcess/get/' + id + '.koala').done(function(json){
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    ...
	                    rs['id']=id;
	  	          		var restring=JSON.stringify(rs);
	  	          		debugger;
	                    $.post('${pageContext.request.contextPath}/ManufactureProcess/update.koala', "data="+restring).done(function(result){
	                        if(result.result == 'success'){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
	    ...
	    Get_Bom: function(id, grid){
	        var self = this;
	        var dialog = $('<div class="modal fade"><div style="width: 100%;" class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">发料</h4></div><div class="modal-body"><p>One fine body&hellip;</p></div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">取消</button><button type="button" class="btn btn-success" id="save">提交</button></div></div></div></div>');
	        $.get('<%=path%>/AfterDaWaferGetBom.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	               self.initPage(dialog.find('form'));
	               $.get( '${pageContext.request.contextPath}/ManufactureProcess/get/' + id + '.koala').done(function(json){
                       json = json.data;
	...
	form.find('#search').on('click', function(){
        ...
        $.ajax({
			...
			debugger;
			grid.next().children().children("#God_qtyTotle").text("Qty:"+result['dieTotal']+" | Strip #:"+result['StripTotal']);
			grid.next().children().children("#God_batchTotle").text(result['countLot']);
		});
    });
});
