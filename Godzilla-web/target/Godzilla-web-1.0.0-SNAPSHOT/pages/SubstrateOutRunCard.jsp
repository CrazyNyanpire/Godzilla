...
<html>
<head>
<title>Substrate Outsourcing Process Traveler</title>
...
<body>
	<script>
		...
		.done(function(msg) {
			...
			$('.start_customerLotNumber').barcode(data['start']['substrateLotNumber'],"code128",{barWidth:1, barHeight:30,showHRI:false});
			$('.start_customerLotNumberN').text(data['start']['substrateLotNumber']);
		...
		/*	for(var i=0;i<data['process'].length;i++)*/
			...
			var i = 0;
			var length = 0;
			//如果是线边仓-产品库存206、委外加工-基板委外发货413、基板委外收货414、基板委外检验415站别则显示委外流程卡
			switch(stationId)
			{
			case 206:
			case 413:
			...
			case 415:
				...
				length = 3;
				display = true;
				document.title = "Substrate Outsourcing Process Traveler";
				break;
			default:
				i = 3;
				length = data['process'].length - 3;
				document.title = "Assembly Process Traveler";
			}
	...
	<%
	int stationIds = Integer.parseInt(request.getParameter("stationId"));
	...
	<div id="waferRunCard_mainContent"  >
		<table style="width:100%;" data-sr="wait 0.5s and enter top and scale up 20%">
			<tr style="text-align:center;">
				<td colspan='4' style="font-size: 18px;border-right:none;"><strong style="margin-left: 9%;">Substrate Outsourcing Process Traveler 基板委外流程卡</strong></td>
			...
			<tr class="bold_title">
				<td class="twtfp">Product Lot Number 产品批号</td>
			...
			<tr>
				...
				<td class="twtfp" colspan='2'><div>Comp. 被动组件数量:：<span class="start_compQty"></span></div><div>Units 单元数量：<span class="start_unitsQty"></span></div><div>Strips 基板数量：<span class="start_stripsQty"></span></div></td>
			...
			<tr class="bold_title">
				<td class="twtfp" colspan='1'>Substrate Lot Number  基板批号</td>
				<td >Lot Type  批次类型</td>
