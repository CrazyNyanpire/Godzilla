				...
				if(result['data']!=undefined)
					{
						...
						{
							for(var a in result['data'][i])
								{
									if(a=="children"||a=="parentId")
										{
											continue;
										}
									if(result['data'][i][a]==null||result['data'][i][a]=="")
										{
											result['data'][i][a]="  -  ";
										}
								}
 ...
 */ + function($) {"use strict";
	...
	Message.DEFAULTS.TEMPLATE = '<div class="alert message" style="min-width: 220px;max-width: 420px; padding: 8px;text-align: left;z-index: 20000;">' + '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' + '<span data-toggle="content" style="font-size:14px; word-wrap:break-word;"></span>&nbsp;&nbsp;</div>';
	var old = $.fn.message;
 ...
 */ + function($) {"use strict";
	...
	Confirm.DEFAULTS.TEMPLATE = '<div class="modal fade"  aria-hidden="false"><div class="modal-dialog" style=" width: 400px;"><div class="modal-content"><div class="modal-header"><button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button><h4 class="modal-title">确认</h4></div><div class="modal-body" style=" padding: 10px 40px;font-size: 16px;color: #d9534f;"><span class="glyphicon glyphicon-warning-sign"></span>&nbsp;&nbsp;&nbsp;<span data-role="confirm-content">确定要删除吗?</span></div><div class="modal-footer"><button data-dismiss="modal" class="btn btn-default" type="button">取消</button><button data-role="confirmBtn" class="btn btn-danger" type="button">确定</button></div></div></div></div>';
	var old = $.fn.confirm;
